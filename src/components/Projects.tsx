    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1 gap-4">
        <div className="flex items-start gap-3">
          <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${p.accent} flex items-center justify-center flex-shrink-0 shadow-md`}>
            <Icon name={p.icon as "Building2"} size={20} className="text-white" />
          </div>
          <div>
            <div className="text-xs text-cyan-400 font-medium mb-1">{p.category}</div>
            <h3 className="text-lg font-bold text-white font-['Oswald'] leading-tight">{p.title}</h3>
          </div>
        </div>
        <p className="text-gray-400 text-sm leading-relaxed flex-1">{p.desc}</p>
        <div className="grid grid-cols-3 gap-2">
          {p.metrics.map((m) => (
            <div key={m.label} className="bg-cyan-500/5 border border-cyan-500/10 rounded-xl p-2.5 text-center">
              <div className="text-lg font-bold text-cyan-400 font-['Oswald']">{m.value}</div>
              <div className="text-gray-500 text-xs leading-tight mt-0.5">{m.label}</div>
            </div>
          ))}
        </div>
        <div className="flex items-center justify-between">
          <div className="flex gap-1.5 flex-wrap">
            {p.tags.map((t) => (
              <span key={t} className="px-2 py-0.5 rounded bg-cyan-500/10 text-cyan-400 text-xs border border-cyan-500/20">
                {t}
              </span>
            ))}
          </div>
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-24 bg-[#0a0f1a] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            Более 500 успешно завершённых проектов в различных отраслях.
            Вот несколько примеров того, что мы уже сделали для наших клиентов.
          </p>
        </div>
