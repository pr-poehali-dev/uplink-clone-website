...
const contactItems = [
  {
    icon: "Phone",
    title: "Телефон",
    lines: ["8 (800) 707-93-03", "Бесплатно по России"],
    link: "tel:+78007079303",
    accent: "from-cyan-400 to-blue-500",
  },
  {
    icon: "Mail",
    title: "Email",
    lines: ["info@uplink-it.ru", "Отвечаем в течение часа"],
  ...
  },
  {
    icon: "MapPin",
    title: "Адрес",
    lines: ["Москва, Россия", "Выезд по всей Москве и МО"],
    link: null,
...
function ContactCard({ item, index }: { item: (typeof contactItems)[0]; index: number }) {
  ...
  return (
    ...
    >
      <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${item.accent} flex items-center justify-center mx-auto mb-4 shadow-lg`}>
        <Icon name={item.icon as "Phone"} size={26} className="text-white" />
      </div>
      <h3 className="text-white font-bold font-['Oswald'] text-lg mb-2">{item.title}</h3>
      {item.link ? (
        <a href={item.link} className="text-gray-300 hover:text-cyan-400 transition-colors text-sm leading-relaxed block">
          {item.lines[0]}
        </a>
      ) : (
...
export default function Contacts({ onContactClick }: ContactsProps) {
  ...
  return (
    <section id="contacts" className="py-24 bg-[#0a0f1a] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-gray-400 max-w-xl mx-auto">
            Готовы ответить на вопросы и подобрать оптимальное решение для вашего бизнеса.
        ...
        <div className="mt-16 text-center">
          <div className="glass-card neon-border rounded-3xl p-10 max-w-2xl mx-auto">
            ...
            <p className="text-gray-400 mb-8">
              Оставьте заявку — мы свяжемся с вами в течение 15 минут и проведём бесплатную консультацию.
            </p>
