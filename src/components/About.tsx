import { useScrollAnimation, useScrollAnimationDelayed } from "@/hooks/useScrollAnimation";
...
function TimelineItem({
  ...
  const { ref, isVisible, animationStyle } = useScrollAnimationDelayed(0.15, index * 100);
  return (
    <div
      ref={ref}
...
function ApproachItem({
  ...
  return (
    <div
      ref={ref}
      className={`flex gap-4 glass-card neon-border rounded-xl p-4 transition-[opacity,transform] duration-700 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}
      style={animationStyle}
    >
...
function TeamCard({
  ...
  return (
    <div
      ref={ref}
      className={`glass-card neon-border rounded-2xl p-5 text-center transition-[opacity,transform] duration-700 hover:scale-105 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
      style={animationStyle}
    >
  ...
  return (
    <section id="about" className="py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        ...
        <div
          ref={teamRef}
          className={`transition-[opacity,transform] duration-700 ${teamVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
