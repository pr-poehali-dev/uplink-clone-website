"""
CMS API — чтение и запись контента сайта (услуги, тарифы, проекты, команда, настройки).
Доступ к записи защищён паролем из cms_settings.
"""
import json
import os
import psycopg2
...
def handler(event: dict, context) -> dict:
    ...
    try:
        # GET /  — весь контент для сайта
        if method == "GET" and (path == "/" or path == ""):
            data = get_all_content(conn)
            return ok(data)

        # POST /save/settings
        if method == "POST" and path == "/save/settings":
            ...
            for key, value in updates.items():
                if key == "admin_password":
                    continue
                cur.execute("UPDATE cms_settings SET value = '%s', updated_at = NOW() WHERE key = '%s'" % (value.replace("'", "''"), key.replace("'", "''")))
            conn.commit()
            cur.close()
            return ok({"ok": True})

        # POST /save/password
        if method == "POST" and path == "/save/password":
            if not check_auth(body, conn):
                return err("Неверный пароль", 401)
        ...
        # POST /save/service
        if method == "POST" and path == "/save/service":
            if not check_auth(body, conn):
                return err("Неверный пароль", 401)
            ...
            if sid:
                ...
                # обновляем пункты: помечаем старые неактивными через is_active=false заменой
                cur.execute("UPDATE cms_service_items SET item_text = item_text WHERE service_id = %s" % int(sid))
                # удалим все пункты и вставим заново через UPDATE + INSERT
                ...
                # оставшиеся старые пункты — обнуляем текст
        ...
        # POST /save/plan
        if method == "POST" and path == "/save/plan":
            if not check_auth(body, conn):
                return err("Неверный пароль", 401)
            plan = body.get("plan", {})
            pid = plan.get("id")
        ...
        # POST /save/project
        if method == "POST" and path == "/save/project":
            if not check_auth(body, conn):
                return err("Неверный пароль", 401)
        ...
        # POST /save/team
        if method == "POST" and path == "/save/team":
            if not check_auth(body, conn):
                return err("Неверный пароль", 401)
